...
<head>
  ...
  <title>Gramin Arogya Seva Sanstha | Category</title>
    ...
    <div class="text-center alert alert-success alert-dismissible" id="success" style="display:none;">
      ...
      <div>
        <form id="fupForm">
          <div class="row">
            <div class="col-md-4">
              <input type="text" name="category" id="category" placeholder="Medicine Category" class="form-control">
            </div>
            <div class="col-md-2">
...
$(document).ready(function() {
    ...
    function fetchData() {
        $.ajax({
            url: 'fetch_data_category.php', // Replace with your actual PHP script name
            type: 'GET',
            dataType: 'json',
                ...
                if (response.success) {
                    ...
                    if (categories.length > 0) {
                        ...
                        $.each(categories, function(index, item) {
                            html += '<tr>';
                            html += '<td>' + (index + 1) + '</td>';
                            html += '<td>' + item.category + '</td>'; // Adjust based on your response structure
                            html += '<td>';
    ...
    // Optionally, you can set an interval to periodically refresh data
    // setInterval(fetchData, 60000); // Refresh every 60 seconds (optional)
});
</script>
