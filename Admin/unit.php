...
<head>
  ...
  <title>Gramin Arogya Seva Sanstha | Unit</title>
    ...
    <section class="content">
      ...
      <div>
        <form id="fupForm" >
          <div class="row">
            <div class="col-md-4">
              <input type="text" name="unit" id="unit" placeholder="Medicine Unit" class="form-control">
            </div>
            <div class="col-md-2">
...
$(document).ready(function() {
    ...
    function fetchData() {
        $.ajax({
            url: 'fetch_data_unit.php', // PHP script to fetch data
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    ...
                    $.each(response.units, function(index, unit) {
                        html += '<tr>';
                        html += '<td>' + (index + 1) + '</td>';
                        html += '<td>' + unit.unit + '</td>';
                        html += '<td>';
                        html += '<a href="#edit' + unit.id + '" class="btn btn-primary btn-sm">Edit</a>';
                        html += '&nbsp;';
                        html += '<a href="#delete' + unit.id + '" class="btn btn-danger btn-sm">Delete</a>';
                        html += '</td>';
                        html += '</tr>';
                    });
