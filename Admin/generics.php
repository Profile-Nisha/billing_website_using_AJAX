...
<head>
  ...
  <title>Gramin Arogya Seva Sanstha | Generics</title>
    ...
    <div class="text-center alert alert-success alert-dismissible" id="success" style="display:none;">
      ...
      <div>
        <form id="fupForm" ">
          <div class="row">
            <div class="col-md-4">
              <input type="text" name="generics" id="generics" placeholder="Medicine Generics" class="form-control">
            </div>
            <div class="col-md-2">
...
$(document).ready(function() {
    ...
    function fetchData() {
        $.ajax({
            url: 'fetch_data_generics.php', // PHP script to fetch data
            type: 'GET',
            dataType: 'json',
            success: function(response) {
                if (response.success) {
                    var html = '';
                    $.each(response.generics, function(index, item) {
                        html += '<tr>';
                        html += '<td>' + (index + 1) + '</td>';
                        html += '<td>' + item.generics + '</td>';
                        html += '<td>';
    ...
    // Handle save button click (if needed)
